...
def car_control():
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.4, steer=0, gear=0))
    time.sleep(6)
    dropped_vehicle.apply_control(carla.VehicleControl(steer=-0.6, gear=0))
    time.sleep(4)
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.4, gear=0))
    time.sleep(4)
    location = dropped_vehicle.get_location()
    print("Car New Location: ",location)
    if (location != "x=66.648193, y=0.766224, z=0.162060"):
        dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.4, gear=0, reverse=True))
        time.sleep(5)
    else:
...
try:
    ...
    car_model = random.choice(get_blueprint_of_world.filter('vehicle.*'))
